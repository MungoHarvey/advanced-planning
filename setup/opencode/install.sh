...
#
# Usage:
#   sh setup/opencode/install.sh --project /path/to/your/project
#   sh setup/opencode/install.sh --global
#   sh setup/opencode/install.sh --dry-run --project /path/to/your/project
#
# What is installed:
...
while [ $# -gt 0 ]; do
    case "$1" in
        ...
        --help|-h)
            echo "Usage:"
            echo "  sh setup/opencode/install.sh --project /path/to/project"
            echo "  sh setup/opencode/install.sh --global"
            echo "  sh setup/opencode/install.sh --dry-run --project /path/to/project"
            echo ""
            ...
            echo "No .opencode/ content is created"
            exit 0
            ;;
        *)
...
ap_write_global_runtime() {
    ...
    if [ "$DRY_RUN" != true ]; then
        ...
        printf '{"schema_version": 1, "source_root": "%s", "version": "%s", "written_by": "setup/opencode/install.sh --global"}\n' \
...
merge_agents_md() {
    ...
    _fence_start="<!-- advanced-planning:opencode:start -->"
    _fence_end="<!-- advanced-planning:opencode:end -->"
...
$_fence_end"
    ...
    if grep -q "$_fence_start" "$_agents_file" 2>/dev/null; then
        ...
        if [ "$_count" -gt 1 ]; then
            echo "ERROR: malformed AGENTS.md - multiple advanced-planning:opencode:start fences" >&2
            exit 1
        fi
...
write_ownership() {
    ...
    # Python does the JSON merge: reads existing (if any), adds "opencode" to
...
if [ "$GLOBAL" = true ]; then
    ...
    say ""
    say "Next steps:"
    say "  OpenCode will discover skills in $GLOBAL_DIR/skills/"
    ...
    cat > "$AP_DIR/runtime.json" <<RUNTIMEEOF
{
  ...
  "version": "$AP_VERSION",
  "written_by": "setup/opencode/install.sh",
  "written_at": "$AP_STAMP"
}
RUNTIMEEOF
