...
#
# Usage:
#   .\setup\codex\install.ps1 -Project C:\path\to\your\project
#   .\setup\codex\install.ps1 -Global
#   .\setup\codex\install.ps1 -Project C:\path\to\your\project -DryRun
#
# What is installed:
...
"@
    ...
    if ($existingContent -match [regex]::Escape($fenceStart)) {
        ...
        if ($matches.Count -gt 1) {
            Write-Error "malformed AGENTS.md - multiple advanced-planning:codex:start fences"
            exit 1
        }
...
if ($Global) {
    ...
    if (-not $DryRun) {
        ...
        $gRuntime = [ordered]@{
            ...
            version        = $gVersion
            written_by     = "setup/codex/install.ps1 -Global"
...
if (-not $DryRun) {
    ...
    $runtime = [ordered]@{
        ...
        source_root    = ConvertTo-ApEmbeddedPath $RepoRoot
        version        = $apVersion
        written_by     = "setup/codex/install.ps1"
        written_at     = (Get-Date).ToUniversalTime().ToString("yyyy-MM-ddTHH:mm:ssZ")
    } | ConvertTo-Json -Depth 3
